...
object Part2ConditionalLogic {
    ...
    fun sumDigits(number: Int): Int {
        return number.toString().filter { it.isDigit() }.sumOf { it.digitToInt() }
    }
    ...
    fun filterOddNumbers(numbers: List<Int>): List<Int> {
        ...
        numbers.forEach { number -> if (number % 2 != 0) newList += number }
        return newList
    }
    ...
    fun transformToLowercase(strings: List<String>): List<String> {
        ...
        strings.forEach { string -> newList += string.lowercase() }
        return newList
    }
    ...
    fun isPalindrome(input: String): Boolean {
        return input.replace(" ", "") == input.replace(" ", "").reversed()
    }
    ...
    fun calculateArea(width: Int = 1, height: Int = 2): Int {
        // Your code here
        return width * height
    }
